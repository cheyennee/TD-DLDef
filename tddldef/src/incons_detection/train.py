...
def __prepare(loss_type: str, optimizer_type: str, training_instances_path: str, ground_truths_path: str, model_path: str, model_info_path: str):
    # 加载模型和数据
    model = keras.models.load_model(model_path) # 加载 .h5
...
def __get_outputs(model, input_objects_names, x,
    ...
    weifen_file = output_dir.split('data')[0] + 'data\\wei_fen.txt'
    ...
    if backend == 'tensorflow':
        ...
        for layer_name in layers_names:
            ...
            shape = target_layer.input_shape[1:]
            inp = tf.random.uniform(shape=((1,) + shape), dtype=tf.float32)

            @tf.function
            def layer_forward(inp):
                return target_layer(inp)

            try:
                numerical_grad, analytic_grad =  tf.test.compute_gradient(layer_forward, [inp])
                max_error = np.max(np.abs(numerical_grad[0] - analytic_grad[0]))
                if max_error > 1e-4:
                    with open(weifen_file, "a") as file:
                        file.write(f"{layer_name}\n")
            except Exception as e:
                print(f'自动微分 Error {e}')
        print('微分检测 Over')
