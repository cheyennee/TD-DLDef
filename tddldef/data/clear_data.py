...
for dataset_name in dataset_list:
    print(f'---------- Remove {dataset_name} -------------')
    wei_fen = Path.cwd() / 'wei_fen.txt'
    if wei_fen.exists():
        wei_fen.unlink()

    output_path = Path.cwd() / f'{dataset_name}_output'
    if output_path.exists():
        shutil.rmtree(output_path)

    report_path = Path.cwd() / f'{dataset_name}_report'
    if report_path.exists():
        shutil.rmtree(report_path)

    db_path = Path.cwd() /  f'{dataset_name}.db'
    if db_path.exists():
